...
```{r}
# Create a subset of the top 100 words in the whole text 
top_100 <- red_headed_league_tidy %>% 
  count(word) %>% 
  arrange(-n) %>% 
  slice(1:100)

# Word cloud of counts for the most frequently used words in the entire text
top_100_cloud <- ggplot(data = top_100,
                        aes(label = word)) +
  geom_text_wordcloud(aes(color = n, size = n), shape = "star") +
  scale_size_area(max_size = 7) +
  scale_color_gradientn(colors = c("purple", "black", "steelblue4")) +
  theme_minimal()

top_100_cloud
```

**Figure 1.** The 100 most frequently used words in Sir Author Conan Doyle's "The Red Headed League". Data: Doyle, A.C. 1892.

```{r}
# Perform sentiment analysis using the "nrc" lexicon on entire text
